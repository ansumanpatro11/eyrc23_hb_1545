...
class ArUcoDetector(Node):
    ...
    def __init__(self):
        ...
        self.pub=[self.pub1,self.pub2,self.pub3]
    ...
    def image_callback(self, msg):
        ...
        corners, ids, rejected = aruco.detectMarkers(self.cv_image, self.aruco_dict, parameters=self.parameters)
        self.detected_ids = ids
        self.detected_corners = corners

    def orientation(self,detected_ids,detected_corners,i):
            # aruco_detector = ArUcoDetector()
            # detected_ids = aruco_detector.detected_ids
            # detected_corners = aruco_detector.detected_corners
       
        
            idx = list(detected_ids).index(i)
...
def main(args=None):
    ...
    while rclpy.ok:
        t1=threading.Thread(target=aruco_detector.orientation,args=[aruco_detector.detected_ids,aruco_detector.detected_corners,1])
        t2=threading.Thread(target=aruco_detector.orientation,args=[aruco_detector.detected_ids,aruco_detector.detected_corners,2])
        t3=threading.Thread(target=aruco_detector.orientation,args=[aruco_detector.detected_ids,aruco_detector.detected_corners,3])
        t1.start()
        t2.start()
        t3.start()
